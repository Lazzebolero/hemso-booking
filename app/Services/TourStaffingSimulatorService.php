...
class TourStaffingSimulatorService
{
    ...
    public function normalizeParams(array $params): array
    {
        ...
        $scheduleMode = (string) ($params['schedule_mode'] ?? 'classic');
        if (! in_array($scheduleMode, ['classic', 'dynamic'], true)) {
    ...
    {
        if (($params['schedule_mode'] ?? 'classic') === 'dynamic') {
            return $this->dynamicMainStarts($day, $params);
        }
    ...
    /**
     * Dynamiskt grundschema: halvtimmesstarter med färjekorrigering på heltimmar
     * (samma princip som klassiskt läge: 11:00 → 11:10 vid +10).
     *
     * @param  array<string, mixed>  $params
     * @return list<Carbon>
     */
    private function dynamicMainStarts(Carbon $day, array $params): array
    {
        $blocks = $this->dynamicMainBlocks($day, $params);
        $times = [];
    ...
    /**
     * Ett huvudslots per halvtimme. Färja ±10 påverkar bara heltimmar.
     *
     * @param  array<string, mixed>  $params
     * @return list<list<Carbon>>
     */
    private function dynamicMainBlocks(Carbon $day, array $params): array
    {
        $first = Carbon::parse($day->toDateString().' '.$params['first_tour']);
        $last = Carbon::parse($day->toDateString().' '.$params['last_tour']);
        ...
        $blocks = [];
        $cursor = $first->copy()->second(0)->microsecond(0);
        ...
        while ($cursor->lte($last)) {
            $anchor = $cursor->copy()->second(0)->microsecond(0);
            $slot = $this->applyFerryToFullHour($anchor, $ferry);

            if ($slot->gte($first) && $slot->lte($last)) {
                $blocks[] = [$slot];
            }
    ...
    private function tryOpenTour(
        ...
        if (($params['schedule_mode'] ?? 'classic') === 'dynamic') {
            return $this->tryOpenDynamicTour(
                $tours,
                $guides,
                $openStarts,
                $createdAt,
        ...
        if ($chosen === null) {
            $chosen = $this->findOverflowExtraSlot(
                $tours,
                $guides,
                $openStarts,
                $createdAt,
            ...
            if ($chosen !== null) {
                return $this->commitOpenedTour($tours, $guides, $chosen, $params, true, true);
            }

            return false;
        }

        return $this->commitOpenedTour($tours, $guides, $chosen, $params, $isExtra15);
    }

    /**
     * Dynamiskt läge: huvudstarter per halvtimme med färjekorrigering på heltimmar.
     * Extratur: högst en per lucka. Lucka ≥30 → +15. Lucka ≥40 → +15 eller +20 (störst nytta).
     *
     * @param  list<array{start: Carbon, end: Carbon, guide_index: int, people: int, capacity: int, bookings: int, is_extra_15: bool, max_guest_wait: int, people_over_max_wait: int, exceeds_max_wait: bool}>  $tours
     * @param  list<array{index: int, available_at: Carbon}>  $guides
     ...
     * @param  array<string, mixed>  $params
     */
    private function tryOpenDynamicTour(
        array &$tours,
        array &$guides,
        array $openStarts,
        Carbon $createdAt,
        ...
        $blocks = $this->dynamicMainBlocks($firstSlot->copy()->startOfDay(), $params);

        $mainSlot = $this->bestFeasibleMainSlotFromBlocks(
            $blocks,
            $guides,
            $openStarts,
            $createdAt,
        ...
        $extraSlot = $this->findDynamicGapExtraSlot(
            $tours,
            $guides,
            $openStarts,
            $createdAt,
        ...
        $mainWait = $mainSlot === null
            ? PHP_INT_MAX
            : $this->queueWaitMinutes($createdAt, $mainSlot, $firstSlot);
        $extraWait = $extraSlot === null
            ? PHP_INT_MAX
            : $this->queueWaitMinutes($createdAt, $extraSlot, $firstSlot);

        // Luck-extratur bara om väntan till nästa huvudtid överskrider max — annars
        // räcker 14:10 trots att 13:50 är några minuter tidigare.
        if ($extraSlot !== null && $mainSlot !== null) {
            if ($mainWait > $params['max_wait_minutes'] && $extraWait <= $mainWait) {
                $chosen = $extraSlot;
                $isExtra = true;
            } else {
                $chosen = $mainSlot;
            }
        } elseif ($extraSlot !== null && $mainSlot === null) {
            $chosen = $extraSlot;
            $isExtra = true;
        } else {
            $chosen = $mainSlot;
        ...
        return $this->commitOpenedTour($tours, $guides, $chosen, $params, $isExtra);
    }
     ...
     * @param  array<string, mixed>  $params
     */
    private function commitOpenedTour(
        array &$tours,
        array &$guides,
        Carbon $chosen,
        array $params,
        bool $isExtra15,
        bool $allowReducedGuides = false,
    ): bool {
        $guideIndex = $this->pickGuide($guides, $chosen, $params, $allowReducedGuides);
        if ($guideIndex === null) {
            return false;
        }
    ...
    /**
     * Välj första lediga halvtimmesblock och bästa variant i blocket (:00/:10 eller :30/:40).
     *
     * @param  list<list<Carbon>>  $blocks
     * @param  list<array{index: int, available_at: Carbon}>  $guides
     * @param  array<string, bool>  $openStarts
     * @param  array<string, mixed>  $params
     */
    private function bestFeasibleMainSlotFromBlocks(
        array $blocks,
        array $guides,
        array $openStarts,
        Carbon $createdAt,
        ...
        foreach ($blocks as $block) {
            ...
            foreach ($block as $option) {
                if (isset($openStarts[$option->format('H:i')])) {
                    $blockTaken = true;
                    break;
                }
            }
            ...
            foreach ($block as $slot) {
                ...
                if ($this->isBetterSlotCandidate($slot, $wait, $bestInBlock, $bestWait, $params)) {
                    $bestInBlock = $slot->copy();
                    $bestWait = $wait;
                }
            }
     ...
     * @param  array<string, mixed>  $params
     */
    private function findDynamicGapExtraSlot(
        array $tours,
        array $guides,
        array $openStarts,
        Carbon $createdAt,
        ...
        foreach ($tours as $tour) {
            ...
            if ($previousMain === null || $tour['start']->gt($previousMain)) {
                $previousMain = $tour['start']->copy();
            }
        }
        ...
        $nextBlock = null;
        foreach ($blocks as $block) {
            $blockStart = $block[0] ?? null;
            if ($blockStart === null || $blockStart->lte($previousMain)) {
                continue;
            }
        ...
        $nextMain = collect($nextBlock)->sortBy(fn (Carbon $slot) => $slot->timestamp)->first();
        if (! $nextMain instanceof Carbon) {
            return null;
        }

        // Högst en extratur per lucka — om något redan ligger mellan huvudtiderna, skippa.
        foreach ($tours as $tour) {
            if ($tour['start']->gt($previousMain) && $tour['start']->lt($nextMain)) {
                return null;
            }
        }

        $gap = (int) $previousMain->diffInMinutes($nextMain, false);
        if ($gap < 30) {
            return null;
        }

        // Lucka 30 → alltid +15. Lucka 40+ → välj EN av +15/+20 där det ger störst nytta.
        $offsets = $gap >= 40 ? [15, 20] : [15];
        $best = null;
        $bestWait = null;
        $minSpacingMinutes = 15;

        foreach ($offsets as $offset) {
            $slot = $previousMain->copy()->addMinutes($offset);

            if ($slot->gte($nextMain)) {
                continue;
            }

            if ($slot->lt($createdAt) || $slot->gt($lastSlot)) {
                continue;
            }
            ...
            if ($this->isTooCloseToExistingTour($tours, $slot, $minSpacingMinutes)) {
                continue;
            }
            ...
            if ($this->isBetterSlotCandidate($slot, $wait, $best, $bestWait, $params)) {
                $best = $slot->copy();
                $bestWait = $wait;
            }
        }
     ...
     * @param  list<array{start: Carbon, end: Carbon, guide_index: int, people: int, capacity: int, bookings: int, is_extra_15: bool, max_guest_wait: int, people_over_max_wait: int, exceeds_max_wait: bool}>  $tours
     */
    private function isTooCloseToExistingTour(array $tours, Carbon $slot, int $minSpacingMinutes): bool
    {
        foreach ($tours as $tour) {
            $distance = abs((int) $tour['start']->diffInMinutes($slot, false));

            if ($distance < $minSpacingMinutes) {
                return true;
            }
        }
    ...
    /**
     * När sista planerade starten är full/otillgänglig: öppna sen extratur efter last_tour.
     * Använder guider som blivit lediga (även efter guide-minskning) och kan fånga bokningar
     * skapade efter sista huvudtid men före den sena starten.
     *
     * @param  list<array{start: Carbon, end: Carbon, guide_index: int, people: int, capacity: int, bookings: int, is_extra_15: bool, max_guest_wait: int, people_over_max_wait: int, exceeds_max_wait: bool}>  $tours
     * @param  list<array{index: int, available_at: Carbon}>  $guides
     * @param  array<string, bool>  $openStarts
     * @param  array<string, mixed>  $params
     */
    private function findOverflowExtraSlot(
        array $tours,
        array $guides,
        array $openStarts,
        Carbon $createdAt,
        ...
        $lastTourStart = null;
        foreach ($tours as $tour) {
            if ($lastTourStart === null || $tour['start']->gt($lastTourStart)) {
                $lastTourStart = $tour['start']->copy();
            }
        }

        $horizon = $lastSlot->copy()->addMinutes(
            (int) $params['duration_minutes'] + (int) $params['buffer_minutes'] + 30
        );

        $earliest = $createdAt->copy();
        if ($lastTourStart !== null) {
            $minAfterLast = $lastTourStart->copy()->addMinutes(15);
            if ($minAfterLast->gt($earliest)) {
                $earliest = $minAfterLast;
            }
        }

        if ($earliest->gt($horizon)) {
            return null;
        }
        ...
        if ($lastTourStart !== null) {
            foreach ([15, 20, 30] as $offset) {
                $slot = $lastTourStart->copy()->addMinutes($offset);
                $candidates[$slot->format('H:i')] = $slot;
            }
        }

        $ferry = (int) ($params['ferry_adjustment'] ?? 0);
        $cursor = $lastSlot->copy()->second(0)->microsecond(0);
        $minute = (int) $cursor->format('i');
        if ($minute > 0 && $minute < 30) {
            $cursor->minute(0);
        } elseif ($minute > 30) {
        ...
        while ($cursor->lte($horizon)) {
            $slot = $this->applyFerryToFullHour($cursor->copy(), $ferry);
            $candidates[$slot->format('H:i')] = $slot;
            $cursor->addMinutes(30);
        }
        ...
        foreach ($candidates as $slot) {
            if ($slot->lt($earliest) || $slot->gt($horizon)) {
                continue;
            }
            ...
            if ($this->isTooCloseToExistingTour($tours, $slot, 15)) {
                continue;
            }

            if ($this->pickGuide($guides, $slot, $params, allowReducedGuides: true) === null) {
                continue;
            }
     ...
     * @param  array<string, mixed>  $params
     */
    private function isBetterSlotCandidate(
        Carbon $slot,
        int $wait,
        ?Carbon $best,
        ?int $bestWait,
        ...
        // Samma väntan: färjekorrigering får väga vid lika, annars tidigare tid.
        $ferry = (int) ($params['ferry_adjustment'] ?? 0);
        $slotMinute = (int) $slot->format('i');
        $bestMinute = (int) $best->format('i');

        if ($ferry === 10) {
            $slotFerryScore = in_array($slotMinute, [10, 40], true) ? 1 : 0;
            $bestFerryScore = in_array($bestMinute, [10, 40], true) ? 1 : 0;
            if ($slotFerryScore !== $bestFerryScore) {
                return $slotFerryScore > $bestFerryScore;
            }
        }

        if ($ferry === -10) {
            $slotFerryScore = in_array($slotMinute, [0, 30], true) ? 1 : 0;
            $bestFerryScore = in_array($bestMinute, [0, 30], true) ? 1 : 0;
            if ($slotFerryScore !== $bestFerryScore) {
                return $slotFerryScore > $bestFerryScore;
            }
        }
     ...
     * @param  array<string, mixed>  $params
     */
    private function pickGuide(array $guides, Carbon $slot, array $params, bool $allowReducedGuides = false): ?int
    {
        $eligibleCount = count($guides);

        if (
            ! $allowReducedGuides
            && is_string($params['guide_reduce_at'] ?? null)
            && $params['guide_reduce_at'] !== ''
        ) {
