...
class TourController extends Controller
{
    public function index(Request $request)
    {
        ...
        $query = Tour::with(['guide', 'tourType', 'bookings.languages']);
    ...
    public function store(Request $request)
    {
        ...
        LogService::log('tour', $tour->id, 'created', null, $tour->toArray(), 'Skapade tur');

        if ($tour->guide_id) {
            app(NotificationService::class)
                ->sendGuideAssigned($tour->fresh(['guide', 'tourType']));
        }
    ...
    public function update(Request $request, Tour $tour)
    {
        ...
        $oldTitle = $tour->title;
        $oldDate = (string) $tour->tour_date;
        $oldStartTime = $tour->start_time;
        $oldEndTime = $tour->end_time;
        ...
        LogService::log('tour', $tour->id, 'updated', $old, $tour->fresh()->toArray(), 'Uppdaterade tur');

        $freshTour = $tour->fresh(['guide', 'tourType']);

        if ($freshTour->guide_id && $freshTour->guide_id != $oldGuideId) {
            app(NotificationService::class)
                ->sendGuideAssigned($freshTour);
        } elseif (
            $freshTour->guide_id &&
            (
                $freshTour->title != $oldTitle ||
                (string) $freshTour->tour_date != $oldDate ||
                $freshTour->start_time != $oldStartTime ||
                $freshTour->end_time != $oldEndTime
            )
        ) {
            app(NotificationService::class)
                ->sendGuideTourUpdated($freshTour);
        }
    ...
    public function destroy(Tour $tour)
    {
        ...
        LogService::log('tour', $tour->id, 'deleted', $old, null, 'Tog bort tur');
