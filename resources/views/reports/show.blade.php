...
@php
    ...
    $reportAttachments = $report->resolvedAttachments();
@endphp
...
<div class="admin-grid-2">
    <div>
        ...
        <div class="page-card mb-4">
            <div class="section-title">Bifogade bilder</div>
                    ...
                    @foreach($reportAttachments as $attachment)
                        @php
                            $attachmentUrl = $attachment->id
                                ? route($reportsRoutePrefix.'.reports.attachments.show', [$report, $attachment])
                                : route($reportsRoutePrefix.'.reports.attachment', $report);
                        @endphp
                        <div class="col-md-6">
                            <div class="info-item">
                                <a href="{{ $attachmentUrl }}" target="_blank" rel="noopener noreferrer" class="btn btn-outline-primary btn-sm mb-2">
                                    <i class="bi bi-box-arrow-up-right me-2"></i>Öppna bild {{ $loop->iteration }}
                                </a>
                                <div class="rounded overflow-hidden border">
                                    <img
                                        src="{{ $attachmentUrl }}"
                                        alt="Bifogad bild {{ $loop->iteration }} till felrapporten"
